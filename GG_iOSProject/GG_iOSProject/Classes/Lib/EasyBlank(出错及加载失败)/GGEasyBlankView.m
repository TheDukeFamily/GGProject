...
@implementation GGEasyBlankView
...
- (void)configWithType:(GGEasyBlankPageViewType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void(^)(UIButton *sender))block
{
    ...
    if (hasError) {
        ...
        if (blankPageType == GGEasyBlankPageViewTypeNodata) {
...
@implementation UIView (GGConfigBlank)

- (void)setBlankPageView:(GGEasyBlankView *)blankPageView{
    objc_setAssociatedObject(self, BlankPageViewKey,
                             blankPageView,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (GGEasyBlankView *)blankPageView{
    return objc_getAssociatedObject(self, BlankPageViewKey);
}

- (void)configBlankPage:(GGEasyBlankPageViewType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
    if (hasData) {
        ...
        if (!self.blankPageView) {
            self.blankPageView = [[GGEasyBlankView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        }
