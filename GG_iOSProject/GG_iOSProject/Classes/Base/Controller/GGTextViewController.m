...
@interface GGTextViewController ()
/** <#digest#> */
...
@implementation GGTextViewController
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField.isEmptyAutoEnable && (GGIsEmpty(textField.text.stringByTrim) || GGIsEmpty(current))) {
        if (GGIsEmpty(current)) {
...
{
    if (GGIsEmpty(self.requiredTextFields)) {
        return;
    }
    
    if ([self respondsToSelector:@selector(textViewControllerRelationButtons:)]) {
        if (GGIsEmpty([self textViewControllerRelationButtons:self])) {
            return;
        }
    }else
    {
    ...
    if (!isEmpty) {
        [self.requiredTextFields enumerateObjectsUsingBlock:^(UITextField *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj != textField && GGIsEmpty(obj.text.stringByTrim)) {
                isButtonEnabled = NO;
                *stop = YES;
            }
        }];
...
@implementation UITextField (GGTextViewController)
...
@implementation GGTextViewControllerTextField
- (NSString *)text {
